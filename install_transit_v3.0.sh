...
# v3.0: [BUGFIX-1] 清理函数必须用可靠的命名，不依赖外部变量
# 使用专用的临时文件前缀，便于精准清理
_transit_cleanup(){
  # 清理本脚本的临时文件（原子写入的 .tmp. 前缀文件）
  ...
  # 清理运行中的临时 map/snippet 文件（当前会话创建的）
  ...
  # 清理 mktemp 生成的文件（会话级别）
  find "${TEMP_DIR}" -maxdepth 1 -type f -name '.transit-mgr.*' -delete 2>/dev/null || true
}

# v3.0: [BUGFIX-1] 正确的 trap 顺序：EXIT 永远包含清理，INT/TERM 在退出前调用清理
# 这样 Ctrl+C、正常退出、错误退出都能正确清理
...
_mktemp(){
  ...
  # 使用 bash 内置 $RANDOM 和 /dev/urandom 生成唯一文件名
  local random_suffix
  ...
  set +e
  (
    timeout "$timeout_secs" mktemp "$dir/.transit-mgr.${prefix}.XXXXXX" 2>/dev/null
  )
  local mktemp_result=$?
  set -"$old_bashopts"
  
  if (( mktemp_result == 0 )) && [[ -n "$mktemp_result" ]]; then
    printf '%s' "$mktemp_result"
    return 0
  fi
  
  # 最终回退：使用 date+pid+random 生成唯一文件名
  printf '%s/.transit-mgr.%s.%d.%s' "$dir" "$prefix" "$$" "$(date +%s%N)"
  return 0
}
...
# v3.0: [架构优化] atomic_write 现在是纯 bash 实现，不依赖子进程 EXIT trap
atomic_write(){
  local target="$1" mode="$2" owner_group="${3:-root:root}"
  local dir tmp
...
have_ipv6(){
  [[ -f /proc/net/if_inet6 ]] \
    ...
    && [[ "$(sysctl -n net.ipv6.conf.all.disable_ipvjsjs_ipv6 2>/dev/null || echo 1)" != "1" ]]
}
...
show_help(){
  cat <<HELP
用法: bash install_transit_v3.0.sh [选项]
  （无参数）        交互式安装或管理菜单
...
_tune_nginx_worker_connections(){
  ...
  if ! grep -qE "^worker_rlimit_nofile\s+${_tune_fd}\s*;" "$mc" 2>/dev/null; then
...
NGINX_STREAM_EOF
  ...
  # 安全追加 stream include 到 nginx.conf
...
setup_firewall_transit(){
  ...
  iptables -A "$FW_TMP" -p icmp --icmp-type echo-request -m limit --limit 10/second --limit-burst 20 -j ACCEPT
  ...
  iptables -A "$FW_TMP" -p tcp  --dport "$LISTEN_PORT"            -j DROP
...
iptables -A ${FW_CHAIN} -p tcp  --dport ${LISTEN_PORT} -m connlimit --connlimit-above 2000 --connlimit-mask 24 -j DROP
iptables -A ${FW_CHAIN} -p tcp  --dport ${LISTEN_PORT} -m connlimit --connlimit-above 20000 --connlimit-mask 0  -j DROP
iptables -A ${FW_CHAIN} -p tcp  --dport ${LISTEN_PORT} -m hashlimit --hashlimit-upto 8000/sec --hashlimit-burst 9999 --hashlimit-mode srcip --hashlimit-name transit_443_limit -j ACCEPT
iptables -A ${FW_CHAIN} -p tcp  --dport ${LISTEN_PORT}                                                        -j DROP
...
generate_nodes(){
  ...
  while IFS= read -r meta; do
    ...
    if [[ -z "$uuid" || -z "$pwd" || -z "$pfx" ]]; then
      warn "节点 ${dom} 缺少 uuid/pwd/pfx（旧版 Token），跳过节点生成"
      continue
    fi
