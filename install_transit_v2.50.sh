#!/usr/bin/env bash
# install_transit_v2.50.sh — 中转机安装脚本 (v2.50 终极版)
# SNI嗅探 → 纯TCP盲传(TFO+KA=3m:10s:3+backlog=65535) → 落地机 | 动态双栈兼容
# 空/无匹配SNI/探测流量 → 17.253.144.10:443（苹果CDN，无DNS解析）· proxy_timeout=315s
set -euo pipefail
IFS=$'\n\t'
...
show_help(){
  cat <<HELP
用法: bash install_transit_v2.50.sh [选项]
...
optimize_kernel_network(){
  ...
  info "优化内核并发参数（拥塞控制权归 BBRPlus/BBR）..."
...
# xray-transit: raised for high-concurrency gRPC — install_transit_v2.50.sh
* soft nofile ${_fd_max}
* hard nofile ${_fd_max}
  ...
  if command -v nginx &>/dev/null; then
    ...
    if command -v apt-get &>/dev/null; then
      ...
      apt-get install -y nginx-common libnginx-mod-stream nginx 2>/dev/null \
        ...
        || die "Nginx 安装失败（apt-get 返回非零），请检查 apt 源或手动安装"
...
_tune_nginx_worker_connections(){
  ...
  local _tune_ram_mb; _tune_ram_mb=$(free -m 2>/dev/null | awk '/^Mem:|^[内內]存:/{print $2}')
  [[ -z "$_tune_ram_mb" ]] && _tune_ram_mb=1024
  
  local _tune_fd=$(( _tune_ram_mb * 800 ))
  (( _tune_fd < 524288 ))   && _tune_fd=524288
  (( _tune_fd > 10485760 )) && _tune_fd=10485760
  
  local _wc_val=$(( _tune_ram_mb / 2 * 1000 ))
  (( _wc_val < 10000 )) && _wc_val=10000
  (( _wc_val > 200000 )) && _wc_val=200000
  ...
  if ! nginx -t 2>/dev/null; then
    ...
    if ! mv -f "$_mc_bak" "$mc" 2>/dev/null; then
      cp -a "$_mc_bak" "$mc" || die "nginx.conf restore FAILED"
    fi
...
SVCOV
  ...
  systemctl daemon-reload \
    || warn "nginx service.d daemon-reload failed"
  if systemctl is-active --quiet nginx 2>/dev/null; then
    # 涉及 LimitNOFILE 等硬性资源修改，必须 restart，reload 无法生效
    systemctl restart nginx 2>/dev/null \
      || warn "Nginx restart 失败"
  fi
  success "Nginx worker_connections=${_wc_val} / worker_rlimit_nofile=${_tune_fd}"
}
...
init_nginx_stream(){
  ...
  local _stream_ram_mb; _stream_ram_mb=$(free -m 2>/dev/null | awk '/^Mem:|^[内內]存:/{print $2}')
  [[ -z "$_stream_ram_mb" ]] && _stream_ram_mb=1024
  
  local _stream_zone_mb=$(( _stream_ram_mb / 32 ))
  (( _stream_zone_mb < 5  )) && _stream_zone_mb=5
  (( _stream_zone_mb > 64 )) && _stream_zone_mb=64

  # 纯字面量 Fallback，完全剥离 DNS 依赖
...
NGINX_STREAM_EOF
  ...
  if ! nginx -t 2>/dev/null; then
    if grep -q 'fastopen=' "$NGINX_STREAM_CONF" 2>/dev/null; then
      ...
      if ! nginx -t 2>/dev/null; then
        ...
        die "Nginx 配置验证失败；nginx.conf 和 stream 配置均已还原"
      fi
  ...
  success "Nginx stream 配置写入完成（空/无匹配SNI→Apple CDN · proxy_timeout=315s）"
}

generate_landing_snippet(){
  local domain="$1" ip="$2" port="${3:-443}"
  local safe; safe=$(domain_to_safe "$domain")
  [[ -n "$safe" ]] || die "域名 safe 转换后为空，拒绝生成 map: ${domain}"
...
import_token(){
  ...
  if ! python3 -c "import ipaddress,sys; ipaddress.IPv4Address(sys.argv[1])" "$ip" 2>/dev/null; then
    die "token 中 ip='${ip}' 不是合法 IPv4 地址！\n  可能原因：落地机生成 Token 时 ip/dom 参数位移"
  fi
...
purge_all(){
  ...
  if [[ -f "$NGINX_MAIN_CONF" ]]; then
    ...
    sed -i '/^\s*worker_connections\s\+[0-9]\+\s*;/d' "$NGINX_MAIN_CONF" 2>/dev/null || true
