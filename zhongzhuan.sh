...
# 🔴 Grok: 空SNI/无匹配SNI 改回落本地 nginx 43231，消除定向苹果CDN固定时序
setup_fallback_decoy_transit(){
  ...
  # v2.46: v1.4: 清除旧版 45232 tls-reject 残留文件（已废弃）
  rm -f "/etc/nginx/conf.d/transit-tls-reject.conf" 2>/dev/null || true
  atomic_write "$fallback_conf" 644 root:root <<FDEOF
# transit-fallback.conf — 内部防探针回落站（45231），由脚本管理
# [v2.7 Gemini-Doc2-🔴] 10m zones / [v2.7 Grok-Doc2-🔴] error_page 400 503 / [v2.7] return 444
# [v2.9 Gemini-D-🔴] listen ... http2 (h2c): Xray terminates TLS then forwards plaintext
# H2 frames to 45231. Without h2c, nginx cannot parse the HTTP/2 preface → distinct protocol
# mismatch error pattern fingerprinted by GFW. With h2c nginx properly accepts and closes.
...
FDEOF
  ...
  success "fallback已由map配置处理（dummy.invalid→Apple CDN)"
}
...
_detect_ssh(){
  ...
  if echo "\$p" | grep -qE '^[0-9]+\$' && [ "\$p" -ge 1 ] && [ "\$p" -le 65535 ]; then
    ...
    logger -t transit-firewall "WARN: 无法动态探测 SSH 端口，使用安装时值 ${ssh_port}"
    echo "${ssh_port}"
  fi
}
...
generate_nodes(){
  ...
  if [[ -z "$transit_ip" ]]; then
    # [Doc3-3] --strict: IP 获取失败直接 die，占位符不进入订阅链路
...
import_token(){
  ...
  if [[ ! -f "$INSTALLED_FLAG" ]]; then
    ...
    _import_install_rollback(){
      ...
      rm -f "/etc/nginx/conf.d/transit-fallback.conf" "$NGINX_STREAM_CONF" "$LOGROTATE_FILE" 2>/dev/null || true
      systemctl disable --now transit-manager-iptables-restore.service 2>/dev/null || true
      rm -f "/etc/systemd/system/transit-manager-iptables-restore.service" 2>/dev/null || true
    ...
    check_deps; optimize_kernel_network; install_nginx; setup_fallback_decoy_transit; init_nginx_stream; setup_firewall_transit
    write_logrotate
...
purge_all(){
  ...
  rm -f  "/etc/nginx/conf.d/transit-fallback.conf" 2>/dev/null || true
  rm -f  "/etc/nginx/conf.d/transit-tls-reject.conf" 2>/dev/null || true
