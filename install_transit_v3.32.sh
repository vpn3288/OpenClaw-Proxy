...
_acquire_lock(){
  mkdir -p "$TEMP_DIR"
  if ! exec {LOCK_FD}>"${TEMP_DIR}/transit-manager.lock" 2>/dev/null; then
    die "无法创建锁文件描述符"
  fi
...
_mktemp(){
  ...
  local random_suffix
  random_suffix=$(LC_ALL=C tr -dc 'a-zA-Z0-9' </dev/urandom 2>/dev/null | head -c 12)
...
show_help(){
  cat <<HELP
用法: bash install_transit_v3.31.sh [选项]
  （无参数）        交互式安装或管理菜单
  --doctor          环境预检（不修改任何内容）
...
setup_fallback_decoy_transit(){
  ...
  # [v3.31] fallback 使用 444 静默关闭连接（不返回任何 TLS 握手，伪装成无服务）
...
stream {
    ...
    map \$ssl_preread_server_name \$backend_upstream {
        ...
        # [v3.31] 恶意 SNI 处理：超长、畸形、非法字符 → 444 静默关闭
        "~^.{254,}"      dummy.invalid;
...
_purge_chain_atomic(){
  local chain="${1}" v="${2:-4}"
  local cmd="${v}tables"
  $cmd-save 2>/dev/null | grep -v "${chain}" | $cmd-restore --noflush --wait 2>/dev/null || true
}
